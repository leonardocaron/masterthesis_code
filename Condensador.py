...
def propriedades_fluido_vec(P_f_in, P_f_out, i_f_in, i_f_out, Q, m_f, BICUBIC_fluido):
    ...
    if Q_vapor > 0.0 and Q_vapor > Q:
        ...
        fracao_vapor = Q_vapor / Q
        fracao_bifasico = 1.0 - fracao_vapor
...
def DP_bifasico(fluido, P_f_in, i_f_in, x_f_in, G_f, d_i_tubo, L_tubo, HEOS_fluido, BICUBIC_fluido):
...
def calor_tubo(G_f, fracao_vapor, fracao_bifasico, eta_aleta, cp_f_in, cp_f_out, \
    ...
    if fracao_vapor == 1.0 and fracao_bifasico == 0.0:  # Somente vapor
        ...
        h_f = h_bifasico(G_f, fluido, d_h_canal, P_f_mean, (1 + x_f_out) / 2, HEOS_fluido, BICUBIC_fluido)
        ...
        DP_bif = DP_bifasico(fluido, P_f_in, i_f_in, (x_f_in + x_f_out) / 2, G_f, d_h_canal, fracao_bifasico * L_elemento, HEOS_fluido, BICUBIC_fluido)
        ...
        DP = DP_bifasico(fluido, P_f_mean, i_f_mean, x_f_mean, G_f, d_h_canal, fracao_bifasico * L_elemento, HEOS_fluido, BICUBIC_fluido)
        ...
        if x_f_mean > 0.02: #Considera bifásico e monofásico
            h_f = h_bifasico(G_f, fluido, d_h_canal, P_f_mean, x_f_mean, HEOS_fluido, BICUBIC_fluido)
        ...
        else: #Considera só monofásico para não dar pau
            h_f = h_monofasico(G_f, fluido, d_h_canal, P_f_mean, i_f_mean, BICUBIC_fluido)
            DP_mono = DP_monofasico(fluido, P_f_mean, (i_f_liq + i_f_out) / 2, G_f, d_h_canal, L_elemento, rug, BICUBIC_fluido)  # Queda de pressão
            DP_bif = 0
...
def condensador(Passes, m_f_total, N_elementos, N_canais, A_canal, P_f_inlet, T_f_inlet, fluido, P_ar_inlet, T_ar_inlet, V_ar_inlet, A_f, sigma, D_h, d_h_canal, P_i_tubo, A_total, A_min, L_tubo):
    ...
    #Segunda estimativa para entalpia
    i_f_ins = np.linspace(CP.PropsSI('H', 'P', P_f_inlet, 'T', T_f_inlet, fluido), CP.PropsSI('H', 'P', P_f_inlet, 'Q', 0, fluido), N_elementos * len(Passes))
    i_f_ins = np.reshape(i_f_ins, (len(Passes), N_elementos))
    i_f_ins = np.repeat(i_f_ins, Passes, axis = 0)
    i_f_in, i_f_out = i_f_ins, i_f_ins - 10e3
    ...
    # Outras variáveis
    fracao_vapor, fracao_bifasico, DP, h_ar, UA, m_ar = np.ones((N_v, N_elementos)), np.zeros((N_v, N_elementos)), np.zeros((N_v, N_elementos)), np.zeros((N_v, N_elementos)), np.zeros((N_v, N_elementos)), np.zeros((N_v, N_elementos))
    ...
    Q_total = sum(sum(Q))
    print("Q_cond: {}".format(Q_total))

    Output = namedtuple('Output', 'Q_total T_f_in T_f_out f_vapor f_bifasico x_in x_out UA i_f_in i_f_out Q P_f_in P_f_out m_f G_f h_f h_ar')
    HX = Output(
        Q_total = Q_total,
        T_f_in = T_f_in,
        T_f_out = T_f_out,
...
def main():
    ...
    Condensador = condensador(Passes, m_f_total, N_elementos, N_canais, A_canal, P_f_inlet, T_f_inlet, fluido, P_ar_inlet, T_ar_inlet, V_ar_inlet, A_f, sigma, D_h, d_h_canal, P_i_tubo, A_total, A_min, L_tubo)
